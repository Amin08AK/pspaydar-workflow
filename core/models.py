...
class User(AbstractUser):
    # ما از فیلدهای پیش‌فرض جنگو مثل ایمیل و نام استفاده می‌کنیم
    # فیلد زیر برای تعیین مدیر هر شخص است
...
class ProcessStep(models.Model):
    ...
    # مسئول پیش‌فرض این مرحله که توسط ادمین قابل تغییر است
    ...
    class Meta:
        ordering = ['process', 'step_order'] # مراحل همیشه مرتب نمایش داده شوند
...
class RequestHistory(models.Model):
    ACTION_TYPES = [
        ...
        ('RESUBMITTED', 'مجدداً ارسال شد'), # <-- این خط را اضافه کنید
    ]
    ...
    class Meta:
        ordering = ['timestamp'] # تاریخچه همیشه به ترتیب زمان نمایش داده شود
